...
int main(int argc, char* argv[]) {
    ...
    /* Begin transaction record */
    data_ptr = wal_begin_record(wal, WAL_RECORD_XACT_COMMIT, xid2,
                                0);  // Note: Type should likely be XACT_BEGIN or similar
    if (!data_ptr || !wal_end_record(wal, NULL)) {
        fprintf(stderr, "Failed to write transaction begin record\n");
        wal_shutdown(wal);
        return 1;
    ...
    /* Begin transaction record */
    data_ptr = wal_begin_record(wal, WAL_RECORD_XACT_COMMIT, xid3,
                                0);  // Note: Type should likely be XACT_BEGIN or similar
    if (!data_ptr || !wal_end_record(wal, NULL)) {
        fprintf(stderr, "Failed to write transaction begin record\n");
        wal_shutdown(wal);
    ...
    printf("\nMonoDB startup completed successfully\n");
    return 0;
}